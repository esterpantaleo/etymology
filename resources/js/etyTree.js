...
function loadTreeFromFile(myWord){
    //hopscotch.endTour();    
    var namefile = "/resources/data/" + myWord.split(' ').join('_') + ".json";
    treeJSON = d3.json(namefile, function(error, treeData) {    
				...
				if ((!d.children[hh].year && (d.children[hh].link === "borrowing" || d.children[hh].link === "abbreviation"))|| (d.children[hh].language === d.language && d.children[hh].year_string && d.children[hh].year_string[0] == "_")) {//if a node has a special link || a node is derived from a word in the same language
				    ...
				    d.children[hh].year_string = "_inferred by parent node because of borrowed/abbreviated/derived from parent word " + d.name;
                        ...
                        for (hh=0; hh<d.name.length; hh++){
                            if (d.name[hh] == myWord && !d.duplicate){
	    ...
	    if (tmpTreeData.length === 0){//if the word is not in the database
                ...
                d3.select("#tree-container")
                    ...
                    .attr("id","message")
                    .attr("align", "center")
                    .html("sorry, word <i>" + myWord + "</i> is not in the database yet");
            } else {
                if (tmpTreeData.length === 1){//if there is only one word in the database matching the searched word
	            ambiguous = false;
                    helpString = "This is the etymological tree of word <br><br><b>" + myWord + "</b>.<br><br><i>Mouseover</i> words to visualize their definition and their language. <br><br>If a node is <i>filled with color</i>, you can click on it.";
                    myTreeData = treeData[0];
                } else { //if there are multiple words in the database matching the searched word
                    ambiguous = true; 
                    helpString = "This is a disambiguation page: word <b>" + myWord + "</b> has multiple entries in the etymology dictionary. <br><br><i>Mouseover</i> words to visualize their definition and their language. <br><br><i>Click</i> on the word that you are interested in.";    
                    myTreeData = {
			name: "_invisible",
			language: "_invisible",
			type: "",
			...
			d3.select("#p-helpPopup").html(helpString);
			    ...
			    } else if (d.link === "compound" || (typeof(d.position) != "undefined" && d.position == "left")){
				return "khaki";
                            } else {
			        return "#fff";
                            }
			    ...
			    if (d._children){
      	      	      	      	return "lightsteelblue";
      	      	      	    } else if (d.link === "compound" || (typeof(d.position) != "undefined" && d.position == "left" || ambiguous)){
                                return	"khaki";
      	      	      	    } else {
                                return "#fff";
      	      	      	    }
		        ...
		        .attr("stroke", function(d) {
		            if (d.target.link == "borrowing" || d.target.link == "abbreviation" || d.target.link == "compound"){
			        return "steelblue";
