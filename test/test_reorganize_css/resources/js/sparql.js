...
function descendantSparql(id) {
    var query = "SELECT DISTINCT ?descendant1 " + // ?descendant2",
        "{ " +
...
function dataSparql(id) {
    var query = "SELECT DISTINCT ?s ?rel ?eq ?der " +
        "{           " +
        "   VALUES ?rel " +
        "   {           " +
        "       <" + id + "> " +
...
function unionSparql(idArray, sparql) {
    var tmp = [];
    idArray.forEach(function(id) { //could do an apply
	    tmp.push(sparql(id));
	});    
