...
function slicedQuery(myArray, mySparql, chunk) {
    ...
    const query = Rx.Observable.zip.apply(this, sources)
        .catch((err) => {
	    displayMainMessage("serverError"); 
...
function appendLanguageText(inner, g){
    inner.selectAll("g.node")
        .append("text")
        .attr("class", ".nodeText")
...
function appendLanguageTooltip(inner, g) {
    inner.selectAll("g.node")
        .append("rect")
        ...
        .on("click", function(d) {
            d3.select("#tooltipPopup").html(function() {
...
function appendDefinitionTooltipOrDisplayDAGRE(inner, g, width, height) {
    var touchtime = 0;
    inner.selectAll("g.node")
        .on('click', function(d) {
            if (touchtime === 0) {
                ...
                if ((new Date().getTime()) - touchtime < 800) {
                    ...
                    displayMainMessage("loading");

                    displayDAGRE(iri, 2, width, height);
                    touchtime = 0;
                } else {
...
function displayDisambiguation(response) {
    if (response !== undefined && response !== null) {
        displayMainMessage("empty");
	d3.select("helpPopup").html(helpMessage["disambiguation"]);

        var graph = JSON.parse(response).results.bindings;
        if (graph.length === 0) {
	    displayMainMessage("notAvailable");
        }
        ...
        var inner = renderGraph(g);
	appendLanguageText(inner, g);
        appendLanguageTooltip(inner, g);
        appendDefinitionTooltipOrDisplayDAGRE(inner, g);
...
function displayDAGRE(iri, parameter) {
    ...
    source.subscribe(
		...
		displayMainMessage("empty");
		d3.select("helpPopup").html(helpMessage["etymology"]);
	    }
            var ancestorArray = [];
                        ...
                        const subscribe = slicedQuery(descendantArray, dataSparql, 8)
                            .subscribe(function(val) {
                                ...
                                if (graphArray.length === 0) {
                                    displayMainMessage("noEtymology");
                                } else {
