	...
	forLoop: for (int j=boundary.start; j<=boundary.end; j++){
		...
		if (currentEtymologyAsPOE.get(j).part.get(k).equals("LEMMA")){
			...
			if (type == 0){
			    ...
			    wdh.registerEtymology(currentEtymologyEntryAsPOE.args, currentEtymologyAsPOE.get(j).args, type);
			} else {//update poe
			    boolean isCompound = wdh.registerEtymology(currentEtymologyEntryAsPOE.args, currentEtymologyAsPOE.get(j).args, type);
